...
export class Pokemon implements Component {
  ...
  description: string;
  stats: object[];
  constructor(name: string, img: string, description: string, stats: Object[]) {
    this.name = name;
    this.img = img;
  ...
  createHtml(): HTMLElement {
	...
	<div id="statsDiv" class="flex-down"></div>
	</div>
	</div>
	...
	for (let i = 0; i < stats.length; i++) {
		...
		containerDiv.querySelector("#statsDiv")?.appendChild(statsParagraph);
	}
    return containerDiv;
  }
}
